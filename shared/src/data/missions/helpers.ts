...
export function calcDeathChance(
  ...
  let chance = noEncounters ? Math.max(0, (mission.difficulty - 1) * 4) : mission.difficulty * 8;
  ...
  // Floor: encounter missions keep a meaningful minimum VIT can't erase.
  // No-encounter missions may legitimately reach 0 (a safe errand).
  chance = noEncounters ? Math.max(chance, 0) : Math.max(chance, 2 + mission.difficulty * 2);
