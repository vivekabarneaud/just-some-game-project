...
describe("meetsRequirements — new gates", () => {
  it("missionCount: gated until the tally reaches the count", () => {
    const req = { missionCount: { id: "tea_at_aldiths", count: 3 } };
    expect(meetsRequirements(req, base)).toBe(false);                                   // no completions
    expect(meetsRequirements(req, { ...base, missionCompletions: { tea_at_aldiths: 2 } })).toBe(false);
    expect(meetsRequirements(req, { ...base, missionCompletions: { tea_at_aldiths: 3 } })).toBe(true);
    expect(meetsRequirements(req, { ...base, missionCompletions: { tea_at_aldiths: 9 } })).toBe(true);
  });
  ...
  it("no requirements → always met; multiple gates must ALL pass", () => {
    ...
    const req = { hasClass: "priest" as const, missionCount: { id: "tea_at_aldiths", count: 3 } };
    expect(meetsRequirements(req, { ...base, rosterClasses: ["priest"] })).toBe(false); // count unmet
    expect(meetsRequirements(req, {
      ...base, rosterClasses: ["priest"], missionCompletions: { tea_at_aldiths: 3 },
    })).toBe(true);
  });
});
