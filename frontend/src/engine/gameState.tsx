...
/** Post-load fixup for a save that PASSED the SAVE_VERSION guard (so it's already
 *  the current shape — no field backfills needed). The one thing still required:
 *  restore the id counter past every id-bearing collection so newly-created ids
 *  don't collide with saved ones, and repair any duplicate ids. Runs on BOTH load
 *  paths. Mutates and returns `saved`. */
