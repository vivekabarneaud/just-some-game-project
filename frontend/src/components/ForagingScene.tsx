...
/** The foraging scene: a painted wood you search by hovering, and a basket you
 *  fill by choosing. Owns ALL of the minigame's state and interaction, and
 *  deliberately touches NO game state — that is what lets the same component
 *  serve both the standalone dev sandbox (which lives outside GameProvider so
 *  the save loop cannot reload it mid-experiment) and the real trip (which is
 *  inside it). The real page passes `onWalkHome` and stores the basket itself.
 *
 *  See docs/design/world/FORAGING_MINIGAME.md.
 *
...
export interface ForagingSceneProps {
  /** Which season's wood to walk into. The sandbox lets you switch it; a real
   *  trip takes the settlement's current season and cannot. */
  season: Season;
  /** Show the tuning chrome: season/seed/rain/refill controls, the mask legend
   *  and the stock readout. Dev sandbox only — the player never sees any of it. */
  sandbox?: boolean;
  /** Real trip: called with the plant ids the player carried home, after Edda
...
export default function ForagingScene(props: ForagingSceneProps) {
  const [season, setSeason] = createSignal<Season>(props.season);
  const [stock, setStock] = createSignal<WoodsStock>(fullStock(props.season));
  ...
  const [walkStock, setWalkStock] = createSignal<WoodsStock>(fullStock(props.season));
  // §3d: a real trip walks into a FULL wood every time — there is no stored
  // stock, and the basket cap is what makes picking a choice. The sandbox still
  // decrements so decay/regrow stay testable.
  createEffect(() => {
    if (props.sandbox) return;
    const s = props.season;
    setSeason(s); setStock(fullStock(s)); setWalkStock(fullStock(s));
  });
          ...
          <Show when={basket().length > 0} fallback={
            ...
            <Show when={!resolved()} fallback={
              <div>
                ...
                {/* A real trip ends by putting the good half in the larder. The
                    binned half never reaches this call: only plants that YIELD
                    something are handed up. */}
                <Show when={props.onWalkHome}>
                  <button style={{ ...BTN, border: "1px solid var(--accent-gold)", "margin-top": "10px" }}
                    onClick={() => props.onWalkHome!(kept().map((b) => b.plantId))}>
                    {props.walkHomeLabel ?? "Put it in the larder"}
                  </button>
                </Show>
              </div>
            }>
