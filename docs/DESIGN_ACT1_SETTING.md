...
- **② Surroundings safe (rich ✓):** The Tainted Spring, `bear_den`→"A Wide Berth", marsh_clearing, alpha_wolf_hunt, spider_hollow, night_howling, stranger_tracks. ~~boar_hunt~~ **stashed**, replaced by the **"Bad Water" boar chain** (below). ~~goblin_shaman_camp~~ **staged** (goblins deferred).
- **The "Bad Water" chain (BUILT: beats 1–3, 2026-06):** the symptom-half of the Tainted Spring side-story. `bad_blood` (a sick boar, mundane) → `bad_season_boars` (too many, the player starts to wonder) → `what_scouts_saw` (the *wrong* ones, met out there: grey-mottled, weeping black, "the death doesn't take" — new `tainted_boar` enemy). Sequential `unique`+`missionDone` chain; two-track discipline (no "water/spring/Aether" named until beat 4). Beats 4 (narrative study, wizard+priest) & 5 (the spring, boss = changed patriarch boar, contain-not-cleanse) still in design. Mercy tone throughout (boars are victims). Art TODO: `tainted_boar` enemy image.
