...
- **Data:** `shared/data/ailments.ts` — `AilmentDef` + `AILMENTS` pool + `BuildingAilment`. **5 ailments** now: *A Bad Cut* & *A Wrenched Back* (injuries, Quarry/Lumber Mill); *A Winter Chill* (winter), *The Summer Gripe* (summer), *The Fen-Ague* (spring/marsh) — all three illnesses contagious + season-weighted, on all founder buildings. So every season has variety.
