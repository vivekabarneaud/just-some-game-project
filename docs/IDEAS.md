...
- **`the_first_merchant` gates on a marketplace *and* a tavern**, which is deep for novice tier, and it's Cobb's *return* — while the merchant design says his first arrival is what prompts building the marketplace. Worth checking that ordering can't deadlock.
