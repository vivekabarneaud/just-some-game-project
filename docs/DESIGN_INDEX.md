...
| FORAGING_PLANTS.md | **WORKING SHEET** (new) | The plant catalogue, filled in one plant at a time: each plant's location, and its ONE identity (a kitchen dish, a tavern drink, an alchemy use, or a decoy — crossover only where real). Carries the locations, the winter test (would you see it in January → painted into the scene, else a sprite), and what gets cut. Started 2026-08-12. → **Work down a location; write each decision here as it is made.** |
| FORAGING_PROMPTS.md | **WORKING SHEET** (new) | The art backlog for the foraging minigame: sprite prompts ordered by value (pairs first, since a pair is one identification test), scene prompts, the terrain-mask painting key, and where every file goes. Written 2026-08-08. → **Work through at leisure; the sandbox picks art up as it appears.** |
| DESIGN_FORAGING_MINIGAME.md | **DESIGNING, not built** (new) | The idle-downtime valve: **one trip a day** (renewable with Orison Shards on an exponential curve, §3b — the earlier no-tickets rule is overturned, because identification needs a scarce trip) into a seasonal hand-painted wood, procedurally scattered plants, **identification-not-eyesight** (no labels ever in-scene; knowledge lives in a **herbier**), no fail state (a decoy costs a basket slot and teaches you). Carries the **plant register**: what's in, what's new (medlar/sloe/elder/crabapple + hemlock/mandrake), the lookalike table, and a **clutter audit** (passive systems punish variety, active ones reward it) that rehomes `nightshade`/`witchs_cap` — two alchemy ingredients currently with **zero sources**. Written 2026-08-07. → **Pick the first fruit + decide the home page, then prototype UI.** |
