...
**The food opening + seasons (corrected again 2026-07-03).** The tick already splits by mode (`gameState.tsx` ~3007): **DEV** drives the season locally by playtime (starts spring, `seasonElapsed` accumulates — this is what you see while testing); **PRODUCTION** derives it from `getGlobalSeason()` — real-world, shared, 3-day seasons — setting `season`, `seasonElapsed`, AND `year` from the global clock. So in prod the farming season IS server/global already, and a real player CAN found in any season. **Winter-start unfairness is therefore real in prod**, and the founding-winter grace (reduce consumption ~0.7×, or a Radiant-One "saint's grace" production buff, flag-gated to the first winter) IS worth having — TODO. Open follow-up: **make `year` local** (settlement age from founding: `global.year - foundingYear + 1`) instead of the shared global year, while keeping season global — "season worldwide, year local."

The actual early-deficit cause was base spring balance + a regression: the adventurers-as-citizens feature made the 3 Thornwood adventurers eat a full adult's 15/h (consumption ~40/h at their arrival vs ~34/h if all three food huts are up). Fix shipped: **`ADVENTURER_FOOD_MULTIPLIER = 0.5`** — adventurers eat half a townsfolk (they forage/provision), dropping the trio to ~7.5/h so the arrival lands near break-even once the food huts are built. Self-diminishes as the town grows. Tunable; validate by playtest.
