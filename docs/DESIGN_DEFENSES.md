...
**The food opening (corrected 2026-07-03).** The *farming* season is currently **local per-save and always starts in spring** (`createInitialState` → `season: "spring"`; it advances by playtime via `seasonElapsed`). Only the cosmetic **weather** reads `getGlobalSeason` (real-world). So there is NO winter-start unfairness today — every opening is spring (1.0× food) — and the winter-grace / rationing idea is moot for now. **Making the farming season server/global is a real refactor** (ripples: aging fires per game-year, farming plant/harvest windows + crop timers key off `seasonElapsed`, offline catch-up, the skip-season dev tool) and is **coupled to the backend/co-op work** — deferred to that pass. IF/when the season goes global, the founding-winter grace (reduce consumption ~0.7× or a Radiant-One "saint's grace" production buff, flag-gated to the first winter) becomes worth adding.
